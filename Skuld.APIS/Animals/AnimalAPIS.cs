...
namespace Skuld.APIS
{
	...
	{
		private readonly Uri BIRDAPI = new("https://random.birb.pw/tweet.json");
		private readonly Uri KITTYAPI = new("https://aws.random.cat/meow");
		private readonly Uri KITTYAPI2 = new("http://thecatapi.com/api/images/get");
		private readonly Uri DOGGOAPI = new("https://random.dog/woof");
		...
		private async Task<string> GetKittyAsync()
		{
			try
			{
				...
				var img = item["file"];
				if (img is null) return "https://i.ytimg.com/vi/29AcbY5ahGo/hqdefault.jpg";
				return img;
			}
			catch
			{
				try
				{
					...
					if (resp is not null)
					{
						if (resp.ResponseUri is not null)
						{
