...
namespace Skuld.Commands
{
    ...
    public class Admin : InteractiveBase
    {
        ...
        public async Task Mute(IUser usertomute)
        {
            ...
            var roleid = await SqlConnection.GetSingleAsync(cmd);
        ...
        public async Task RemoveAutoRole()
        {
            ...
            cmd.Parameters.AddWithValue("@guildid", guild.Id);
            var reader = await SqlConnection.GetSingleAsync(cmd);
            if (reader != null)
            {
                cmd = new MySqlCommand("UPDATE guild SET autojoinrole = NULL WHERE ID = @guildid");
                cmd.Parameters.AddWithValue("@guildid", guild.Id);
                await SqlConnection.InsertAsync(cmd).ContinueWith(async x =>
                {
        ...
        public async Task SetPrefix(string prefix)
        {
            ...
            string oldprefix = await SqlConnection.GetSingleAsync(cmd);
            ...
            await SqlConnection.InsertAsync(cmd).ContinueWith(async x =>
            {
                ...
                cmd.Parameters.AddWithValue("@guildid", Context.Guild.Id);
                var result = await SqlConnection.GetSingleAsync(cmd);
                if (result != oldprefix)
            ...
            else
            {
                ...
                if (settings.ContainsKey(module) || settings.ContainsValue(module))
                {
                    if (!String.IsNullOrEmpty(await SqlTools.GetSingleAsync(new MySqlCommand("SELECT `ID` from `guildfeaturemodules` WHERE `ID` = " + Context.Guild.Id))))
                    {
                        ...
                        await SqlConnection.InsertAsync(new MySqlCommand($"UPDATE `guildfeaturemodules` SET `{setting.Value}` = {value} WHERE `ID` = {Context.Guild.Id}"));
                        if (value == 0)
        ...
        public async Task ConfigureGuildModules(string module, int value)
        {
            ...
            else
            {
                ...
                {
                    if (!String.IsNullOrEmpty(await SqlConnection.GetSingleAsync(new MySqlCommand("SELECT `ID` from `guildcommandmodules` WHERE `ID` = " + Context.Guild.Id))))
                    {
                        await SqlConnection.InsertAsync(new MySqlCommand($"UPDATE `guildcommandmodules` SET `{module}` = {value} WHERE `ID` = {Context.Guild.Id}"));
                        if (value == 0)
        ...
        public async Task ConfigureChannel(string module, IChannel channel)
        {
            ...
            if (modules.ContainsKey(module)||modules.ContainsValue(module))
            {
                if (!String.IsNullOrEmpty(await SqlConnection.GetSingleAsync(new MySqlCommand("SELECT `ID` from `guildcommandmodules` WHERE `ID` = " + Context.Guild.Id))))
                {
                    ...
                    await SqlConnection.InsertAsync(new MySqlCommand($"UPDATE `guild` SET `{setting.Value}` = {channel.Id} WHERE `ID` = {Context.Guild.Id}"));
