...
namespace Skuld.Bot.Commands
{
	...
	public class GamblingModule : InteractiveBase<ShardedCommandContext>
	{
		...
		private readonly Dictionary<string, string> coinflip = new()
		{
			{ "SKULD_COINFLIP_HEADS", "https://static.skuld.bot/img/flip/heads.png" },
			{ "SKULD_COINFLIP_TAILS", "https://static.skuld.bot/img/flip/tails.png" }
		};

		private readonly Dictionary<RPSThrow, string> rps = new()
		{
		...
		private static readonly Dictionary<SlotIcon, string> slotIcons = new()
		{
			{ SlotIcon.Bell, "🔔" },
			{ SlotIcon.Cherry, "🍒" },
			{ SlotIcon.Crown, "👑" },
				...
				case "t":
					{
						...
						await
							...
							.WithDescription($"Result are: {Locale.GetLocale(user.Language).GetString(res.Item1, CultureInfo.InvariantCulture)} {suffix}")
		...
		public async Task RPS(string shoot, ulong bet)
		{
			...
			if (playerThrow != RPSThrow.Invalid)
			{
				...
				var throwName = Locale.GetLocale(user.Language).GetString(rps.FirstOrDefault(x => x.Key == skuldThrow).Value, CultureInfo.InvariantCulture);
		...
		public async Task Slots(ulong bet)
		{
			...
			TransactionService.DoTransaction(new TransactionStruct
			{
				...
				.IsSuccessAsync(async _ => await DoSlotsAsync(bet, MoneyPrefix))
		...
		async Task DoSlotsAsync(ulong bet, string MoneyPrefix)
		{
			using var Database = new SkuldDbContextFactory().CreateDbContext();
			var user = await Database.InsertOrGetUserAsync(Context.User).ConfigureAwait(false);
		...
		[
			...
			Ratelimit(20, 1, Measure.Minutes),
			Remarks("🃏 Play a game of Mia")
		]
		public class MiaModule : InteractiveBase<ShardedCommandContext>
		{
			const string PenaltyNotice = "**Note:** Re-rolling costs 25% of your bet up to 75%";
			...
			public async Task MiaRoll()
			{
				...
				try
				{
					...
					try
					{
						if (session.PreviousMessage is not null)
						{
							await
