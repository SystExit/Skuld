...
namespace Skuld.Bot.Commands
{
	...
	public class AdminModule : InteractiveBase<ShardedCommandContext>
	{
		...
		[Name("Server"), Group("guild"), RequireUserPermission(GuildPermission.ManageGuild)]
		...
		[Name("Starboard"), Group("starboard"), RequireDatabase, RequireRole(AccessLevel.ServerMod)]
		...
		public class Twitter : InteractiveBase<ShardedCommandContext>
		{
			...
			[Command("unfollow"), Summary("Unfollows a twitter user"), RequireDatabase]
			[Usage("#superawesomeupdates skuldbot")]
			...
			[Command("message"), Summary("Sets the notification message. Keys are: -name & -url"), RequireDatabase]
		...
		[Name("Twitch"), Group("twitchstreams")]
		[RequireUserPermission(GuildPermission.ManageGuild)]
		...
		{
			[Command("follow"), Summary("Follows a twitch streamer"), RequireDatabase]
			[Usage("#superawesomeupdates skuldbot")]
			...
			[Command("unfollow"), Summary("Unfollows a twitch streamer"), RequireDatabase]
			[Usage("#superawesomeupdates skuldbot")]
			public async Task UnfollowTwitchUsers(IMessageChannel channel, params string[] users)
			{
				...
				foreach (var usr in usrs.Matches)
				{
					if (database.TwitchFollows.Any(z => z.Streamer == usr.Name))
					{
						database.Remove(database.TwitchFollows.FirstOrDefault(z => z.ChannelId == channel.Id && z.Streamer == usr.Name));
					}
				}
			...
			public async Task UnfollowTwitchUsers(params string[] users)
			{
				...
				foreach (var usr in usrs.Matches)
				{
					if (database.TwitchFollows.Any(z => z.Streamer == usr.Name))
					{
						database.Remove(database.TwitchFollows.FirstOrDefault(z => z.ChannelId == Context.Channel.Id && z.Streamer == usr.Name));
					}
				}
